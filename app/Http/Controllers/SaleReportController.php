...
class SaleReportController extends Controller
{
     ...
     * Les SaleItems reflètent déjà l'état correct après échange :
     * - Items retournés ont exchanged_at (exclus via whereNull)
     * - Items partiellement échangés ont la quantité réduite
     * - Nouveaux items d'échange existent comme SaleItems (added_via_exchange_id)
     */
    ...
    public function store(Request $request, Supplier $supplier)
    {
        ...
        // If nothing was sold the report has nothing to invoice or pay — mark it as
        // already invoiced + paid so the user can still edit/print it but no payment
        // validation step is required.
        $isZeroReport = (float) $totalPayAmount <= 0;
