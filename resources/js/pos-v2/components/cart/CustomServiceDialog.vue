...
function submit() {
    ...
    if (!description.value.trim() || isNaN(parsed) || parsed <= 0) return;
    emit('submit', { description: description.value.trim(), amount: parsed });
}
</script>
            ...
            <div v-if="open" class="fixed inset-0 z-40 flex items-center justify-center p-4 bg-stone-900/40 backdrop-blur-sm" @click.self="emit('close')">
                <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-6">
                    <div class="flex items-center justify-between mb-4">
                        <h3 class="font-serif text-2xl">Custom service</h3>
                    ...
                    <input
                        v-model="description"
                        type="text"
                        placeholder="e.g. Garment alteration"
                        class="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 h-12 text-[15px] mb-4 focus:border-stone-400 focus:outline-none focus:ring-4 focus:ring-stone-200/50"
                    >

                    <label class="block text-sm font-medium text-stone-700 mb-2">Amount</label>
                    <div class="flex items-baseline gap-2 mb-3">
                        <span class="text-stone-400 text-xl">$</span>
                        <div class="font-serif text-3xl tabular-nums">{{ amount === '' ? '0.00' : amount }}</div>
                    </div>
                    <NumPad v-model:value="amount" />
                    ...
                    <div class="flex gap-3 mt-5">
                        ...
                        <button type="button" @click="submit" :disabled="!description.trim() || amount === ''" class="flex-1 h-12 bg-stone-900 hover:bg-stone-800 text-white rounded-xl font-semibold disabled:opacity-50">Add to cart</button>
                    </div>
                </div>
            </div>
        </Transition>
