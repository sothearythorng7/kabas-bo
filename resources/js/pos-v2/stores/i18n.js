import { defineStore } from 'pinia';
import { ref, computed } from 'vue';

const MESSAGES = {
    en: {
        signIn: 'Sign in',
        enterPin: 'Enter your PIN',
        invalidPin: 'Invalid PIN',
        signingIn: 'Signing in…',
        startShift: 'Start shift',
        initialCash: 'Initial cash in drawer',
        popupEvent: 'Popup event (optional)',
        none: 'None',
        todayAbsences: "Today's absences",
        noAbsences: 'No absences today',
        endShift: 'End shift',
        visitorsCount: 'How many visitors today?',
        countedCash: 'Counted cash in drawer',
        verification: 'Verification',
        openingCash: 'Opening cash',
        cashSales: 'Cash sales',
        cashIn: 'Cash in',
        cashOut: 'Cash out',
        expectedCash: 'Expected cash',
        countedAmount: 'Counted amount',
        difference: 'Difference',
        next: 'Next',
        back: 'Back',
        confirm: 'Confirm',
        cancel: 'Cancel',
        cashier: 'Cashier',
        switchCashier: 'Switch cashier',
        addCashIn: 'Cash in',
        addCashOut: 'Cash out',
        amount: 'Amount',
        ok: 'OK',
        loading: 'Loading…',
        offline: 'Offline — using cached data',
        retry: 'Retry',
        payment: 'Payment',
        pay: 'Pay',
        single: 'Single',
        split: 'Split',
        method: 'Method',
        addLine: 'Add line',
        voucher: 'Voucher',
        applyVoucher: 'Apply voucher',
        voucherCode: 'Voucher code',
        validate: 'Validate',
        invalidVoucher: 'Invalid or expired voucher',
        voucherAvailable: 'Available',
        expires: 'Expires',
        totalDue: 'Total due',
        totalPaid: 'Paid',
        remaining: 'Remaining',
        confirmPayment: 'Confirm payment',
        paymentMismatch: 'Split amount does not equal total',
        emptyCart: 'Cart is empty',
        syncedNow: 'Synced',
        syncFailed: 'Sync failed — will retry',
        forceSync: 'Sync now',
        journal: 'Journal',
        date: 'Date',
        search: 'Search',
        searchSales: 'Search sales',
        noResults: 'No sales found for this date',
        salesCount: 'Sales count',
        itemsSold: 'Items sold',
        totalPaidLabel: 'Total paid',
        totalDiscount: 'Total discount',
        saleNo: 'Sale #',
        seller: 'Seller',
        items: 'Items',
        reprint: 'Reprint',
        exchange: 'Exchange',
        beforeDiscount: 'Before discount',
        deliveryAddress: 'Delivery address',
        deliveryFee: 'Delivery fee',
        exchanges: 'Exchanges',
        returnedItems: 'Returned items',
        newItems: 'New items',
        balance: 'Balance',
        holdSale: 'Hold',
        resume: 'Resume',
        lookupSale: 'Look up sale',
        enterSaleId: 'Enter sale number',
        saleNotFound: 'Sale not found',
        saleTooOld: 'Sale is older than 30 days, exchange not allowed',
        selectReturnItems: 'Select items to return',
        selectNewItems: 'Select new items',
        noNewItems: 'No new items selected — straight return',
        returnTotal: 'Return total',
        newItemsTotal: 'New items total',
        balanceCredit: 'Credit due to customer',
        balanceToPay: 'Amount due from customer',
        evenSwap: 'Even swap',
        autoVoucher: 'A voucher will be generated for the credit',
        processExchange: 'Process exchange',
        exchangeDone: 'Exchange recorded',
        notes: 'Notes',
        voucherGenerated: 'Voucher generated',
        soldFor: 'Sold for',
        purchased: 'Purchased',
        daysAgo: 'days ago',
        notExchangeable: 'Not exchangeable',
        planning: 'Planning',
        myPlanning: 'My planning',
        requestLeave: 'Request leave',
        leaveType: 'Leave type',
        startDate: 'Start date',
        endDate: 'End date',
        reason: 'Reason',
        reasonOptional: 'Reason (optional)',
        submit: 'Submit',
        leaveBalance: 'Leave balance',
        availableDays: 'Available days',
        usedDays: 'Used days',
        pendingRequests: 'Pending requests',
        leaveRequested: 'Leave requested — pending approval',
        invalidDateRange: 'Invalid date range',
        leaveConflict: 'These dates overlap with an existing leave',
        leaveTypes: {
            vacation: 'Vacation',
            sick: 'Sick leave',
            personal: 'Personal',
            other: 'Other',
        },
        shiftPlanned: 'Shift planned',
        onLeave: 'On leave',
        today: 'Today',
        prevMonth: 'Previous',
        nextMonth: 'Next',
    },
    fr: {
        signIn: 'Connexion',
        enterPin: 'Saisissez votre PIN',
        invalidPin: 'PIN invalide',
        signingIn: 'Connexion…',
        startShift: 'Ouvrir la caisse',
        initialCash: 'Fond de caisse initial',
        popupEvent: 'Événement (optionnel)',
        none: 'Aucun',
        todayAbsences: "Absences du jour",
        noAbsences: 'Aucune absence aujourd\'hui',
        endShift: 'Fermer la caisse',
        visitorsCount: 'Combien de visiteurs aujourd\'hui ?',
        countedCash: 'Espèces comptées en caisse',
        verification: 'Vérification',
        openingCash: 'Fond initial',
        cashSales: 'Ventes espèces',
        cashIn: 'Entrée caisse',
        cashOut: 'Sortie caisse',
        expectedCash: 'Caisse attendue',
        countedAmount: 'Montant compté',
        difference: 'Écart',
        next: 'Suivant',
        back: 'Retour',
        confirm: 'Confirmer',
        cancel: 'Annuler',
        cashier: 'Caissier',
        switchCashier: 'Changer de caissier',
        addCashIn: 'Entrée caisse',
        addCashOut: 'Sortie caisse',
        amount: 'Montant',
        ok: 'OK',
        loading: 'Chargement…',
        offline: 'Hors-ligne — données en cache',
        retry: 'Réessayer',
        payment: 'Paiement',
        pay: 'Payer',
        single: 'Simple',
        split: 'Scindé',
        method: 'Méthode',
        addLine: 'Ajouter une ligne',
        voucher: 'Bon d\'achat',
        applyVoucher: 'Appliquer un bon',
        voucherCode: 'Code du bon',
        validate: 'Valider',
        invalidVoucher: 'Bon invalide ou expiré',
        voucherAvailable: 'Disponible',
        expires: 'Expire',
        totalDue: 'À payer',
        totalPaid: 'Payé',
        remaining: 'Reste',
        confirmPayment: 'Valider le paiement',
        paymentMismatch: 'Le total scindé ne correspond pas au total',
        emptyCart: 'Panier vide',
        syncedNow: 'Synchronisé',
        syncFailed: 'Sync échouée — réessai automatique',
        forceSync: 'Synchroniser',
        journal: 'Journal',
        date: 'Date',
        search: 'Rechercher',
        searchSales: 'Rechercher des ventes',
        noResults: 'Aucune vente pour cette date',
        salesCount: 'Nombre de ventes',
        itemsSold: 'Articles vendus',
        totalPaidLabel: 'Total encaissé',
        totalDiscount: 'Remises totales',
        saleNo: 'Vente n°',
        seller: 'Vendeur',
        items: 'Articles',
        reprint: 'Réimprimer',
        exchange: 'Échange',
        beforeDiscount: 'Avant remise',
        deliveryAddress: 'Adresse de livraison',
        deliveryFee: 'Frais de livraison',
        exchanges: 'Échanges',
        returnedItems: 'Articles retournés',
        newItems: 'Nouveaux articles',
        balance: 'Solde',
        holdSale: 'Mettre en attente',
        resume: 'Reprendre',
        lookupSale: 'Rechercher une vente',
        enterSaleId: 'Numéro de vente',
        saleNotFound: 'Vente introuvable',
        saleTooOld: 'Vente datant de plus de 30 jours, échange impossible',
        selectReturnItems: 'Articles à retourner',
        selectNewItems: 'Nouveaux articles',
        noNewItems: 'Aucun nouvel article — simple retour',
        returnTotal: 'Total retours',
        newItemsTotal: 'Total nouveaux articles',
        balanceCredit: 'Crédit pour le client',
        balanceToPay: 'À régler par le client',
        evenSwap: 'Échange équivalent',
        autoVoucher: 'Un bon d\'achat sera émis pour le crédit',
        processExchange: 'Valider l\'échange',
        exchangeDone: 'Échange enregistré',
        notes: 'Notes',
        voucherGenerated: 'Bon d\'achat émis',
        soldFor: 'Vendu',
        purchased: 'Acheté',
        daysAgo: 'jour(s)',
        notExchangeable: 'Non échangeable',
        planning: 'Planning',
        myPlanning: 'Mon planning',
        requestLeave: 'Demander un congé',
        leaveType: 'Type de congé',
        startDate: 'Date de début',
        endDate: 'Date de fin',
        reason: 'Raison',
        reasonOptional: 'Raison (optionnel)',
        submit: 'Envoyer',
        leaveBalance: 'Solde congés',
        availableDays: 'Jours disponibles',
        usedDays: 'Jours utilisés',
        pendingRequests: 'En attente',
        leaveRequested: 'Demande envoyée — en attente de validation',
        invalidDateRange: 'Plage de dates invalide',
        leaveConflict: 'Ces dates chevauchent un congé existant',
        leaveTypes: {
            vacation: 'Congés payés',
            sick: 'Maladie',
            personal: 'Personnel',
            other: 'Autre',
        },
        shiftPlanned: 'Shift planifié',
        onLeave: 'En congé',
        today: 'Aujourd\'hui',
        prevMonth: 'Précédent',
        nextMonth: 'Suivant',
    },
};

export const useI18nStore = defineStore('i18n', () => {
    const locale = ref(loadLocale());

    const t = computed(() => (key) => {
        const bundle = MESSAGES[locale.value] || MESSAGES.en;
        return bundle[key] ?? MESSAGES.en[key] ?? key;
    });

    function setLocale(next) {
        if (!MESSAGES[next]) return;
        locale.value = next;
        try { localStorage.setItem('pos_v2_locale', next); } catch { /* ignore */ }
    }

    return { locale, t, setLocale };
});

function loadLocale() {
    try {
        const stored = localStorage.getItem('pos_v2_locale');
        if (stored && MESSAGES[stored]) return stored;
    } catch { /* ignore */ }
    const nav = (navigator.language || 'en').slice(0, 2);
    return MESSAGES[nav] ? nav : 'en';
}
