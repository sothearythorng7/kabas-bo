...
<div class="container mt-4">
    ...
    <form action="{{ isset($account) ? route('stores.accounts.update', [$site, $account]) : route('stores.accounts.store', $site) }}" method="POST">
        ...
        <div class="mb-3">
            <label for="type" class="form-label">{{ __('messages.journals.type') }}</label>
        ...
        <a href="{{ route('stores.accounts.index', $site) }}" class="btn btn-secondary">{{ __('messages.btn.cancel') }}</a>
    </form>
</div>
@endsection
