...
<div class="container mt-4">
    <h1 class="crud_title">Modifier l'Article</h1>
    ...
    <form action="{{ route('blog.posts.update', $post) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
                ...
                <div class="tab-content border border-top-0 p-3 mb-3">
                    @foreach($locales as $index => $locale)
                        <div class="tab-pane fade @if($index===0) show active @endif" id="{{ $locale }}" role="tabpanel">
                            {{-- Titre --}}
                            <div class="mb-3">
                                <label class="form-label">Titre ({{ strtoupper($locale) }}) <span class="text-danger">*</span></label>
                            ...
                            {{-- Extrait --}}
                            <div class="mb-3">
                                <label class="form-label">Extrait ({{ strtoupper($locale) }})</label>
                            ...
                            {{-- Contenu --}}
                            <div class="mb-3">
                                <label class="form-label">Contenu ({{ strtoupper($locale) }}) <span class="text-danger">*</span></label>
                            ...
                            {{-- Meta Title --}}
                            <div class="mb-3">
                                <label class="form-label">Meta Title ({{ strtoupper($locale) }})</label>
                            ...
                            {{-- Meta Description --}}
                            <div class="mb-3">
                                <label class="form-label">Meta Description ({{ strtoupper($locale) }})</label>
            ...
            <div class="col-md-4">
                ...
                <div class="mb-3">
                    ...
                    <small class="form-text text-muted">Ctrl+clic pour sélectionner plusieurs tags</small>
                ...
                <div class="mb-3">
                    ...
                    <div class="position-relative">
                        ...
                        <form action="{{ route('blog.posts.deleteImage', $post) }}" method="POST" class="d-inline" onsubmit="return confirm('Supprimer cette image ?')">
                            @csrf
                            @method('DELETE')
                ...
                {{-- Nouvelle image --}}
                <div class="mb-3">
                    <label class="form-label">{{ $post->featured_image ? 'Remplacer l\'image' : 'Image à la une' }}</label>
                ...
                <div class="card mb-3">
                    <div class="card-body">
                        ...
                        <p class="mb-1"><strong>Vues:</strong> {{ $post->views_count }}</p>
                        <p class="mb-1"><strong>Auteur:</strong> {{ $post->author->name ?? '-' }}</p>
                        <p class="mb-0"><strong>Créé:</strong> {{ $post->created_at->format('d/m/Y H:i') }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mt-4">
            ...
            <a href="{{ route('blog.posts.index') }}" class="btn btn-secondary">Annuler</a>
        </div>
    </form>
</div>
@endsection
