...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="crud_title">{{ __('messages.blog_post.title_show') }}</h1>
        <div class="d-flex gap-2">
            <a href="{{ route('blog.posts.index') }}" class="btn btn-secondary">
                <i class="bi bi-arrow-left"></i> {{ __('messages.blog_post.back_to_list') }}
            </a>
            <a href="{{ route('blog.posts.edit', $post) }}" class="btn btn-primary">
                <i class="bi bi-pencil"></i> {{ __('messages.blog_post.edit') }}
            </a>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-body">
            {{-- Statut --}}
            <div class="mb-3">
                <strong>{{ __('messages.blog_post.status') }}:</strong>
            ...
            <div class="mb-3">
                <strong>{{ __('messages.blog_post.published_date') }}:</strong>
            ...
            @if($post->category)
            <div class="mb-3">
                <strong>{{ __('messages.blog_post.category') }}:</strong>
            ...
            {{-- Auteur --}}
            <div class="mb-3">
                <strong>{{ __('messages.blog_post.author') }}:</strong>
                {{ $post->author->name ?? '-' }}
            </div>

            {{-- Nombre de vues --}}
            <div class="mb-3">
                <strong>{{ __('messages.blog_post.views') }}:</strong>
                {{ $post->views_count }}
            </div>
            ...
            <div class="mb-3">
                <strong>{{ __('messages.blog_post.tags') }}:</strong>
            ...
            <div class="mb-3">
                <strong>{{ __('messages.blog_post.featured_image') }}:</strong>
                <div class="mt-2">
            ...
            <div class="tab-content border border-top-0 p-3">
                @foreach($locales as $index => $locale)
                    <div class="tab-pane fade @if($index===0) show active @endif" id="{{ $locale }}" role="tabpanel">
                        {{-- Titre --}}
                        <div class="mb-3">
                            <strong>{{ __('messages.blog_post.title_label') }}:</strong>
                            <div>{{ $post->title[$locale] ?? '-' }}</div>
                        </div>

                        {{-- Slug --}}
                        <div class="mb-3">
                            <strong>{{ __('messages.blog_post.slug') }}:</strong>
                            <div><code>{{ $post->slug[$locale] ?? '-' }}</code></div>
                        </div>

                        {{-- URL publique --}}
                        <div class="mb-3">
                            <strong>{{ __('messages.blog_post.public_url') }}:</strong>
                            <div>
                        ...
                        {{-- Excerpt --}}
                        <div class="mb-3">
                            <strong>{{ __('messages.blog_post.excerpt') }}:</strong>
                            <div>{{ $post->excerpt[$locale] ?? '-' }}</div>
                        </div>

                        {{-- Contenu --}}
                        <div class="mb-3">
                            <strong>{{ __('messages.blog_post.content') }}:</strong>
                        ...
                        <div class="mb-3">
                            <strong>{{ __('messages.blog_post.meta_title') }}:</strong>
                        ...
                        <div class="mb-3">
                            <strong>{{ __('messages.blog_post.meta_description') }}:</strong>
