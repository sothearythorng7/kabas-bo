...
<div class="container mt-4">
  <h1 class="crud_title">{{ __('messages.hero_slide.title_edit') }}</h1>
  <form action="{{ route('hero-slides.update', $heroSlide) }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-3">
      <label class="form-label">{{ __('messages.hero_slide.current_image') }}</label>
      <div>
    ...
    <div class="mb-3">
      <label class="form-label">
        @if($heroSlide->image_path)
          {{ __('messages.hero_slide.replace_image') }}
        @else
          {{ __('messages.hero_slide.image') }}
        @endif
      </label>
    ...
    <div class="row">
      <div class="col-md-3 mb-3">
        <label class="form-label">{{ __('messages.hero_slide.order_label') }}</label>
      ...
      <div class="col-md-3 mb-3">
        <label class="form-label">{{ __('messages.hero_slide.active_label') }}</label><br>
        <div class="form-check form-switch">
          ...
          <label class="form-check-label" for="is_active">{{ __('messages.hero_slide.active') }}</label>
        </div>
      </div>
      <div class="col-md-3 mb-3">
        <label class="form-label">{{ __('messages.hero_slide.starts_at') }}</label>
      ...
      <div class="col-md-3 mb-3">
        <label class="form-label">{{ __('messages.hero_slide.ends_at') }}</label>
    ...
    <button type="submit" class="btn btn-success">
      <i class="bi bi-save"></i> {{ __('messages.hero_slide.update') }}
    </button>
    <a href="{{ route('hero-slides.index') }}" class="btn btn-secondary">
      <i class="bi bi-x-circle"></i> {{ __('messages.hero_slide.cancel') }}
    </a>
  </form>
</div>
@endsection
