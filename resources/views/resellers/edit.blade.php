...
<div class="container mt-4">
    <h1 class="crud_title">@t('resellers.title_edit')</h1>
    ...
    <form action="{{ route('resellers.update', $reseller) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="type" class="form-select" required>
                <option value="buyer" @selected($reseller->type=='buyer')>@t('resellers.type_buyer')</option>
                <option value="consignment" @selected($reseller->type=='consignment')>@t('resellers.type_consignment')</option>
            </select>
        </div>
        <button type="submit" class="btn btn-success">
            <i class="bi bi-save"></i> @t('resellers.update')
        </button>
