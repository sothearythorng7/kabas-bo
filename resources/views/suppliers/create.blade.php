...
<div class="container mt-4">
    ...
    <form action="{{ route('suppliers.store') }}" method="POST">
        ...
        {{-- Nouveau champ type --}}
        <div class="mb-3">
            <label for="type" class="form-label">{{ __('messages.supplier.type') }}</label>
            <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                <option value="buyer" {{ old('type') === 'buyer' ? 'selected' : '' }}>{{ __('messages.supplier.type_buyer') }}</option>
                <option value="consignment" {{ old('type') === 'consignment' ? 'selected' : '' }}>{{ __('messages.supplier.type_consignment') }}</option>
            </select>
